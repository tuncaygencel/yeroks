...
$connect->enter_active(1,1,0,0,0);
$sql="SELECT id,name,latitude,longitude FROM province";
...
<meta name="description" content="Yeroks ile Çevrendeki Binlerce Mağaza ve AVM'nin Ürünlerini Arayabilir ve Onlara Ulaşabilirsin!..."/>
<link href="css/index_real4.css" type="text/css" rel="stylesheet"/>
...
var page=1;
var s_icon ={
				url:"https://www.yeroks.com/img_files/loc_marker2.png",
...
function get_shop_category_special(cat_id)
	...
	function text_typing(){
			...
			function input_change_to_box(v,i){
				$('#search_area').html('<div id="search_text_box"><div id="y_icon" style="background-image:url(\'https://www.yeroks.com/y_img_small/'+i+'.png\')"></div><div id="search_text_box_text">'+v+'</div><div OnClick="search_text_box_exit()" id="search_text_box_exit"></div></div>');
				};
				
			function search_text_box_exit(){
				$('#search_area').html('<div id="search_in_place_input_outline"><input type="text" onkeydown="text_typing()" onkeyup="text_typing()" placeholder="Gömlek, Pizza, Bilgisayar......" id="search_in_place_input"/></div>');
					...
					$.ajax({
						...
						if(yerok.type!=1)
								...
								distance=(parseFloat(result.distance).toFixed(2))*1000+" m";
									...
									$('#s_result').append("<div id=\"r_main\"><div id=\"r_shop_main\"><div id=\"r_shop_icon\"></div><div id=\"r_shop_text\" onclick=\"window.open('https://www.yeroks.com/shop.php?shop_no="+result.s_id+"');\" >"+result.s_name+"</div></div><div id=\"r_avm_main\"><div id=\"r_distance\">"+distance+"</div></div></div>");				
									}else{
									$('#s_result').append("<div id=\"r_main\"><div id=\"r_shop_main\"><div id=\"r_shop_icon\"></div><div id=\"r_shop_text\" onclick=\"window.open('https://www.yeroks.com/shop.php?shop_no="+result.s_id+"');\" >"+result.s_name+"</div></div><div id=\"r_avm_main\"><div id=\"r_avm_text\" onclick=\"window.open('https://www.yeroks.com/place.php?place_no="+result.place_id+"');\" >"+result.place_name+"</div><div id=\"r_distance\">"+distance+"</div></div></div>");
		...
		function  marker_enter(latitude,longitute,s_name,s_data,place_id,place_name,distance,marker_id){
				...
				markers.push(marker);
				if(place_id==0){
				addInfowindow("<div id=\"r_main\"><div id=\"r_shop_main\"><div id=\"r_shop_icon\"></div><div id=\"r_shop_text\" onclick=\"window.open('https://www.yeroks.com/shop.php?shop_no="+s_data+"');\" >"+s_name+"</div></div><div id=\"r_avm_main\"><div id=\"r_distance\">"+distance+"</div></div></div>");				
				}else{
				addInfowindow("<div id=\"r_main\"><div id=\"r_shop_main\"><div id=\"r_shop_icon\"></div><div id=\"r_shop_text\" onclick=\"window.open('https://www.yeroks.com/shop.php?shop_no="+s_data+"');\" >"+s_name+"</div></div><div id=\"r_avm_main\"><div id=\"r_avm_text\" onclick=\"window.open('https://www.yeroks.com/place.php?place_no="+place_id+"');\" >"+place_name+"</div><div id=\"r_distance\">"+distance+"</div></div></div>");
		...
		function get_exp_yields(type,cat_num){
		$('#y_icons').html('<img height=40 style="margin:15px;" src="https://www.yeroks.com/img_files/cycle_loading.gif"/>');
		$.ajax({
					type:'POST',
		...
		for (i = 1; i < 43; i++) { 
			if(i !== cat_num){
			$('#cat_exp_'+i).css('backgroundColor' , 'white');
			}
		}
		$('#cat_exp_'+cat_num).css('backgroundColor' , 'orange');
		}
   ...
   function display_exp()
    ...
    function bottom_scroll()
		{
			var dis_scroll=0;
			var des_des=120+$('#search_area_main').height();
	...
	function left_scroll()
		{
		 $('#y_icons_area').animate( { scrollLeft: '+=300' }, 200);
		return false;
		}
...
<div class="log_l_ic1"></div><div class="log_l_text">Giriş Yap</div>
</div>
<div id="login_link" onclick="location.href='https://www.yeroks.com/shop_index.php'">
<div class="log_l_ic2"></div><div class="log_l_text">Mağazanı Kaydet</div>
</div>
</div>
</div>
</div>
...
<div class="y_text">Ürün Seç ve Çevrende Ara...</div>
	<div id="y_icons">
<?php
$sql="SELECT DISTINCT y_type_id,icon_name,y_name FROM y_name_list WHERE  is_y=1 and icon_name!=0 GROUP BY icon_name ORDER BY RAND() LIMIT 0,60";
$sql=mysql_query($sql) or die(mysql_error());

while($res=mysql_fetch_assoc($sql))
	{
	echo "<div id=\"yield_show\" onclick=\"change_value('".$res['y_name']."',".$res['y_type_id'].",".$res['icon_name'].",2)\"><img src=\"https://www.yeroks.com/y_img_small/".$res['icon_name'].".png\" height=40 title=\"".$res['y_name']."\" id=\"e_y_icon\" /><div id=\"yield_show_text\">".$res['y_name']."</div></div>";
	}
?>
</div>
</div>
...
<div id="y_exp_change">
<div id="y_exp_input">
  <div onclick="get_exp_yields(0,1)" id="cat_exp_1" style="background-color:orange" class="cat_exp"><div id="cat_exp_text">Tüm Kategoriler</div></div>
  <div onclick="get_exp_yields(0,30)" id="cat_exp_30" class="cat_exp"><div id="cat_exp_text">Yiyecek</div></div>
  <div onclick="get_exp_yields(0,31)" id="cat_exp_31" class="cat_exp"><div id="cat_exp_text">İçecek</div></div>
  <div onclick="get_exp_yields(0,32)" id="cat_exp_32" class="cat_exp"><div id="cat_exp_text">Yemek Aparatları</div></div>
  <div onclick="get_exp_yields(0,40)" id="cat_exp_40" class="cat_exp"><div id="cat_exp_text">Bayan Giyim</div></div>
  <div onclick="get_exp_yields(0,41)" id="cat_exp_41" class="cat_exp"><div id="cat_exp_text">Erkek Giyim</div></div>
  <div onclick="get_exp_yields(0,42)" id="cat_exp_42" class="cat_exp"><div id="cat_exp_text">Çocuk Giyim</div></div>
  <div onclick="get_exp_yields(0,4)" id="cat_exp_4" class="cat_exp"><div id="cat_exp_text">Kozmetik</div></div>
  <div onclick="get_exp_yields(0,5)" id="cat_exp_5"  class="cat_exp"><div id="cat_exp_text">Elektronik</div></div>
  <div onclick="get_exp_yields(0,6)" id="cat_exp_6" class="cat_exp"><div id="cat_exp_text">Ev Gereçleri</div></div>
  <div onclick="get_exp_yields(0,7)" id="cat_exp_7" class="cat_exp"><div id="cat_exp_text">Dekorasyon-Mobilya</div></div>
  <div onclick="get_exp_yields(0,8)" id="cat_exp_8" class="cat_exp"><div id="cat_exp_text">Kitap-Dergi</div></div>
  <div onclick="get_exp_yields(0,9)" id="cat_exp_9" class="cat_exp"><div id="cat_exp_text">Market Ürünleri</div></div>
  <div onclick="get_exp_yields(0,10)" id="cat_exp_10" class="cat_exp"><div id="cat_exp_text">Spor, Outdoor</div></div>
  <div onclick="get_exp_yields(0,11)" id="cat_exp_11" class="cat_exp"><div id="cat_exp_text">Oyuncak ve Eğlence</div></div>
</div>
